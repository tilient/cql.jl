...
function cqlConnect(srv::String = "localhost", prt::Int = 9042)
...
function cqlDisconnect(con::CQLConnection)
  while 0 < con.pending
    yield();
  end
  close(con.socket);
...
function cqlQuery(con::CQLConnection, msg::String)
  ...
  cqlSync(con);
  cqlAsyncGetResult(cqlAsyncQuery(con, msg))
end

function cqlCommand(con::CQLConnection, msg::String)
  ## The same as 'cqlQuery', but we don't get the result back.
  ## Is a bit faster and uses less memory, because the
  ## reply from the server is neglected.
  cqlSync(con);
  cqlAsyncCommand(con, msg)
  cqlSync(con);
  nothing
end

function cqlAsyncQuery(con::CQLConnection, msg::String)
  ...
  ## 'cqlAsyncGetResult', which gives back the result in 
  ## the same format as 'cqlQuery'.
  id, reply = nextReplyID(con);
  sendMessage(con, 0x07, msg, id);
  reply
end

function cqlAsyncCommand(con::CQLConnection, msg::String)
  ## The same as 'cqlCommand', but asynchronous.
...
function cqlAsyncGetResult(reply)
  ## To fetch the result from a call by cqlAsyncQuery.
  ## Will block if the result is not there yet.
  decodeMessage(take!(reply) ...)
end

function cqlSync(con)
...
function test01()
  ...
  c = cqlConnect("tilient.net");

  cqlCommand(c, "CREATE KEYSPACE demo 
                 WITH REPLICATION = {'class' : 'SimpleStrategy',
                                     'replication_factor' : 1};");
  cqlCommand(c, "USE DEMO;");
  ...
  println(cqlQuery(c, "SELECT * FROM SONGS"));
  println(cqlQuery(c, "SELECT id, tags FROM songs;"));
  println(cqlQuery(c, "SELECT id, venue FROM songs;"));
  ...
  cqlCommand(c, "DROP TABLE songs;");
  cqlCommand(c, "DROP TABLE playlists;");
  cqlCommand(c, "DROP KEYSPACE demo;");
...
function test02()
  ...
  cqlCommand(c, "CREATE KEYSPACE demo 
                 WITH REPLICATION = {'class' : 'SimpleStrategy',
                                     'replication_factor' : 1};");
  cqlCommand(c, "USE DEMO;");
  cqlCommand(c, "create table person (id int primary key, 
                                      name varchar); ");
  ...
  println(cqlQuery(c, "SELECT count(*) from person;")[1][1]);

  cqlCommand(c, "DROP TABLE person;");
  cqlCommand(c, "DROP KEYSPACE demo;");
  cqlDisconnect(c);
  nothing
end
...
function test03()
  ...
  cqlCommand(c, "CREATE KEYSPACE demo 
                 WITH REPLICATION = 
                   {'class' : 'SimpleStrategy',
                    'replication_factor' : 1};");
  cqlCommand(c, "USE DEMO;");
  cqlCommand(c, "create table person (id int , 
                                      od int,
                                      name varchar, 
                              PRIMARY KEY (id, od)); ");
  ...
  @time begin
    for i in 1:N
      cqlAsyncCommand(c, "INSERT INTO person (id, od, name) 
                           VALUES (1, $i, 'hihaho');");
    end
    cqlSync(c);
  end
  ...
  @time begin
    cqlQuery(c, "select id, od from person 
                  where id = 1
