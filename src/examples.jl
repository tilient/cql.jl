...
function test01()
  ...
  c = cql.connect("tilient.net");

  cql.command(c, "CREATE KEYSPACE demo 
...
function test02()
  ...
  cql.command(c, "CREATE KEYSPACE demo 
                 WITH REPLICATION = {'class' : 'SimpleStrategy',
                                     'replication_factor' : 1};");
  cql.command(c, "USE DEMO;");
  cql.command(c, "create table person (id int primary key, 
                                      name varchar); ");
...
function test03()
  ...
  cql.command(c, "CREATE KEYSPACE demo 
                 WITH REPLICATION = 
                   {'class' : 'SimpleStrategy',
                    'replication_factor' : 1};");
  cql.command(c, "USE DEMO;");
  cql.command(c, "create table person (id int , 
                                      od int,
                                      name varchar, 
                              PRIMARY KEY (id, od)); ");
  ...
  @time begin
    for i in 1:N
      cql.asyncCommand(c, "INSERT INTO person (id, od, name) 
                           VALUES (1, $i, 'hihaho');");
    end
    cql.sync(c);
  end
  ...
  @time begin
    cql.query(c, "select id, od from person 
                  where id = 1
